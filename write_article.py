...
def get_content_prompt(section_type, section_title, keyword, synonyms_list=None, search_intent="معلوماتية", article_goal=""):
    ...
    if "معلوماتية" in search_intent:
      intent_rules.append("النية (معلوماتية Informational): القارئ يريد التعلم. اشرح المصطلحات المعقدة ببساطة، تعمق في التفاصيل، واستخدم أمثلة توضيحية لتثقيفه.")
    if "ملاحية" in search_intent:
      intent_rules.append("النية (ملاحية Navigational): القارئ يبحث عن وجهة محددة. كن مباشراً جداً، وجهه للمعلومة أو الرابط أو اسم المنصة الرسمية التي يبحث عنها بدون مقدمات.")
    if "استقصائية" in search_intent or "مقارنة" in search_intent:
      intent_rules.append("النية (استقصائية/مقارنة Commercial): القارئ يقارن قبل الشراء. كن محايداً، ركز على الفروقات الجوهرية (المميزات/العيوب/السعر)، وسهل عليه اتخاذ القرار.")
    if "شرائية" in search_intent:
      intent_rules.append("النية (شرائية Transactional): القارئ جاهز للدفع. ركز على القيمة، الفوائد السريعة، الأمان، وشجعه بأسلوب واثق ومقنع لاتخاذ قرار الشراء/الاشتراك.")
        
    # 2. النوايا التفصيلية
    if "محلية" in search_intent:
      intent_rules.append("النية (محلية Local): اربط المعلومات بالسياق المحلي الجغرافي، ركز على الأماكن أو الخدمات القريبة والمتاحة فعلياً.")
    if "معلومة موجزة" in search_intent:
      intent_rules.append("النية (معلومة موجزة Know Simple): القارئ في عجلة من أمره. أعطه 'الزبدة' والجواب النهائي الثابت (رقم، اسم، تاريخ) في أول جملة بدون أي حشو.")
    if "وسائط" in search_intent:
      intent_rules.append("النية (وسائط Multimedia): القارئ يريد صوراً أو فيديوهات. صف له ما يجب أن يراه بصرياً، أو وجهه لكيفية الحصول على المحتوى المرئي بسهولة.")
    if "مجاني" in search_intent:
      intent_rules.append("النية (بحث مجاني Freebie): القارئ لا يريد الدفع. ركز كلياً على الأدوات المجانية 100%، البدائل المفتوحة المصدر، وكيفية التوفير وتخطي القيود المدفوعة.")
        
    # 3. النوايا السلوكية والمتقدمة
    if "إجرائية" in search_intent or "أفعل" in search_intent:
      intent_rules.append("النية (إجرائية Actionable): القارئ يريد التنفيذ الآن. اكتب بأسلوب (1، 2، 3)، استخدم أفعال أمر (اضغط، حمل، ادخل)، وقدم خطوات تقنية فورية.")
    if "مختلطة" in search_intent:
      intent_rules.append("النية (مختلطة Split): الكلمة لها عدة معاني. قم بتغطية جميع الزوايا المحتملة للكلمة لإرضاء جميع أنواع القراء المحتملين.")
    if "حداثة" in search_intent or "تريند" in search_intent:
      intent_rules.append("النية (حداثة News): القارئ يبحث عن التريند. اكتب بأسلوب صحفي سريع، ركز على التحديثات اللحظية، وما الذي تغير (اليوم/هذا العام).")
    if "مشكلات" in search_intent or "حل" in search_intent:
      intent_rules.append("النية (حل مشكلات Troubleshooting): القارئ غاضب ولديه عطل. تقمص دور المهندس المنقذ، اشرح سبب المشكلة باختصار، ثم اطرح الحلول الفعالة مباشرة.")
    if "ترفيهية" in search_intent:
      intent_rules.append("النية (ترفيهية Entertainment): القارئ يريد التسلية. استخدم أسلوباً مرحاً، طريفاً، خفيف الظل، واجعل المحتوى ممتعاً ومسلياً للقراءة.")
        
    # 4. لحظات جوجل الاستراتيجية
    if "أذهب" in search_intent:
      intent_rules.append("النية (أريد أن أذهب): القارئ يخطط لزيارة. ركز على العناوين، النصائح المكانية، تقييمات الزوار، والتفاصيل العملية للزيارة.")
    if "موسمية" in search_intent:
      intent_rules.append("النية (موسمية Seasonal): القارئ يبحث عن مناسبة (كرمضان أو البلاك فرايداي). استخدم نبرة حماسية ترتبط بالحدث، وركز على العروض واستغلال الوقت.")
    if "ضمنية" in search_intent:
      intent_rules.append("النية (ضمنية Implicit): اقرأ ما بين السطور لما يبحث عنه القارئ وقدم إجابات تلبي احتياجه الخفي الذي لم يصرح به مباشرة.")
